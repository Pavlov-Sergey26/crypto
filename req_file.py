...
def request_coinbase(url, endpoint):
    ...
    #Считывание данных с приватном файле
...
#Для старта я взял недельный интервал и время просмотра 1 час(для примера)
futures = "ETH-USDT"
...
url_candles = f"https://api.coinbase.com/api/v3/brokerage/products/{futures}/candles?start={dt_7days}&end={dt_now}&granularity=ONE_HOUR"
url_price = f"https://api.coinbase.com/api/v3/brokerage/products/{futures}"

#Вызов функции
request_coinbase_candles = request_coinbase(url_candles, f"/api/v3/brokerage/products/{futures}/candles")
request_coinbase_price = request_coinbase(url_price, f"/api/v3/brokerage/products/{futures}")


#Подключение базы данных MongoDB (Так же подключение и данные я сохранил в папке .env)
load_dotenv(find_dotenv())
